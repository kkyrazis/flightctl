...
func validateApplications(apps []ApplicationProviderSpec, fleetTemplate bool) []error {
	...
	for _, app := range apps {
		...
		case ArtifactApplicationProviderType:
			...
			if lo.FromPtr(app.AppType) != AppTypeQuadlet {
				allErrs = append(allErrs, fmt.Errorf("artifact application provider does not support %q application type", lo.FromPtr(app.AppType)))
			}
			// TODO add support for appropriate AppTypes
