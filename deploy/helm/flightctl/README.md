...
The defaults are 2 minutes between runs, 1000 devices per page, and 30 seconds of work per invocation. Page sizes can be
configured up to 1000. The time budget is checked between complete pages; the saved cursor resumes the scan on a later
run.
...
| periodic | object | `{"clusterLevelSecretAccess":false,"consumers":5,"image":{"image":"quay.io/flightctl/flightctl-periodic-el9","pullPolicy":"","tag":""},"metrics":{"address":":15690","enabled":true},"tasks":{"labelMappingScan":{"pageSize":1000,"schedule":{"interval":"2m"},"timeBudget":"30s"}}}` | Periodic Configuration |
