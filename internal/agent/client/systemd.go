...
type SystemdEnableOption func(opts *enableOpts)
type SystemdDisableOption func(opts *enableOpts)

type enableOpts struct {
	now bool
}

func WithEnableNow() SystemdEnableOption {
	return func(opts *enableOpts) {
		opts.now = true
	}
}

func WithDisableNow() SystemdDisableOption {
	return func(opts *enableOpts) {
		opts.now = true
	}
}

func (s *Systemd) Disable(ctx context.Context, name string, opts ...SystemdDisableOption) error {
	return s.DisableMany(ctx, []string{name}, opts...)
}

func (s *Systemd) DisableMany(ctx context.Context, units []string, opts ...SystemdDisableOption) error {
	var eOpts enableOpts
	...
	_, stderr, exitCode := s.exec.ExecuteWithContext(ctx, systemctlCommand, args...)
	if exitCode != 0 {
		return fmt.Errorf("disable systemd unit(s): %s: %w", strings.Join(units, ","), errors.FromStderr(stderr, exitCode))
	}
	return nil
}
...
func (s *Systemd) EnableMany(ctx context.Context, units []string, opts ...SystemdEnableOption) error {
	var eOpts enableOpts
	for _, opt := range opts {
		opt(&eOpts)
	}
	...
	_, stderr, exitCode := s.exec.ExecuteWithContext(ctx, systemctlCommand, args...)
	if exitCode != 0 {
		return fmt.Errorf("enable systemd unit(s): %s: %w", strings.Join(units, ","), errors.FromStderr(stderr, exitCode))
	}
	return nil
}
