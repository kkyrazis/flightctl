...
const (
	...
	helmPullAuthPath   = "/root/.config/containers/auth.json"

	dryRunTimeout = 90 * time.Second
)
...
func helmPullOptions(resolver dependency.PullConfigResolver) dependency.ClientOptsFn {
	...
	return resolver.Options(
		dependency.PullConfigSpec{
			Paths:    []string{helmRegistryConfig, helmPullAuthPath},
...
func (p *helmProvider) Verify(ctx context.Context) error {
	...
	dryRunOpts := []client.HelmOption{
		...
		client.WithHelmTimeout(dryRunTimeout),
		client.WithValuesFiles(valuesPaths),
	}
...
func (p *helmProvider) collectOCITargets(ctx context.Context, configProvider dependency.PullConfigResolver) (dependency.OCIPullTargetsByUser, error) {
...
func (p *helmProvider) extractNestedTargets(ctx context.Context, configProvider dependency.PullConfigResolver) (*AppData, error) {
	...
	for _, img := range images {
		targets = append(targets, dependency.OCIPullTarget{
			...
			Reference:    img,
			PullPolicy:   v1beta1.PullIfNotPresent,
			ClientOptsFn: criPullOptions(configProvider),
		})
	}
...
func (p *helmProvider) parentIsAvailable(ctx context.Context) (string, string, bool, error) {
	resolved, err := p.clients.Helm().IsResolved(p.chartRef())
	if err != nil {
		return "", "", false, fmt.Errorf("check chart resolved: %w: %w", errors.WithElement(p.chartRef()), err)
	}
	if !resolved {
