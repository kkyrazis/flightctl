...
// verifyAndLabelTPMEnrollment verifies a TCG CSR and stores the verification status in labels.
// This method always allows enrollment to proceed, storing any errors in labels for admin review.
func (h *ServiceHandler) verifyAndLabelTPMEnrollment(er *api.EnrollmentRequest, csrBytes []byte, name string) {
	...
	if err := tpm.VerifyTCGCSRChainOfTrustWithRoots(csrBytes, trustedRoots); err != nil {
		// store verification failure reason but allow enrollment to proceed
		(*er.Metadata.Labels)["flightctl.io/tpm-verification-status"] = "failed"
		(*er.Metadata.Labels)["flightctl.io/tpm-verification-error"] = err.Error()
		h.log.Warnf("TPM verification failed for enrollment request %s: %v", name, err)
	} else {
		(*er.Metadata.Labels)["flightctl.io/tpm-verification-status"] = "passed"
...
func (h *ServiceHandler) createDeviceFromEnrollmentRequest(ctx context.Context, orgId uuid.UUID, enrollmentRequest *api.EnrollmentRequest) error {
	...
	isTPMVerified := false
	if enrollmentRequest.Metadata.Labels != nil {
		if status, ok := (*enrollmentRequest.Metadata.Labels)["flightctl.io/tpm-verification-status"]; ok {
			isTPMVerified = (status == "passed")
		}
	}
	...
	if isTPMVerified {
		...
		if tpm.IsTCGCSRFormat(csrBytes) {
			...
			if enrollmentRequest.Metadata.Labels != nil {
				if errMsg, ok := (*enrollmentRequest.Metadata.Labels)["flightctl.io/tpm-verification-error"]; ok {
					if strings.Contains(errMsg, "TPM CA certificates not configured") {
						...
						tpmErrorMsg = fmt.Sprintf("TPM verification failed: %s", errMsg)
						deviceIdentityMsg = fmt.Sprintf("Identity verification failed: %s", errMsg)
					}
				}
			}
