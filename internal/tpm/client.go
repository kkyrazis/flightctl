...
func (t *Client) createPCRPolicySession(pcrSelection *tpm2.TPMLPCRSelection) ([]byte, error) {
	...
	startAuthRsp, err := startAuthCmd.Execute(transport.FromReadWriter(t.conn))
	if err != nil {
		return nil, fmt.Errorf("starting policy session: %w", err)
	}

	sessionHandle := startAuthRsp.SessionHandle
	defer func() { _ = t.flushContextForHandle(sessionHandle) }()
	...
	_, err = policyPCRCmd.Execute(transport.FromReadWriter(t.conn))
	if err != nil {
	...
	policyGetDigestRsp, err := policyGetDigestCmd.Execute(transport.FromReadWriter(t.conn))
	if err != nil {
...
func (t *Client) calculatePCRPolicyDigest(pcrSelection *tpm2.TPMLPCRSelection) ([]byte, error) {
	policyDigest, err := t.createPCRPolicySession(pcrSelection)
	if err != nil {
...
func (t *Client) createEndorsementPrimary() (*tpm2.NamedHandle, error) {
	createPrimaryCmd := tpm2.CreatePrimary{
		PrimaryHandle: tpm2.AuthHandle{
...
func (t *Client) sealStoragePassword(password []byte) error {
	if len(password) == 0 {
	...
	policyDigest, err := t.calculatePCRPolicyDigest(pcrSelection)
	if err != nil {
		return fmt.Errorf("calculating PCR policy digest: %w", err)
	}

	endorsementPrimary, err := t.createEndorsementPrimary()
	if err != nil {
		return err
	}
	defer func() { _ = t.flushContextForHandle(endorsementPrimary.Handle) }()
	...
	err = t.persistence.saveSealedPasswordBlob(createRsp.OutPublic, createRsp.OutPrivate, pcrSelection)
	if err != nil {
...
func (t *Client) unsealStoragePassword() ([]byte, error) {
	public, private, pcrSelection, err := t.persistence.loadSealedPasswordBlob()
	if err != nil {
	...
	defer func() { _ = t.flushContextForHandle(loadRsp.ObjectHandle) }()
	...
	unsealRsp, err := unsealCmd.Execute(transport.FromReadWriter(t.conn))
	if err != nil {
