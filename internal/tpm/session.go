...
func (s *tpmSession) Close() error {
	...
	// TODO: when we are in a shared env we will need to make this conditional if we hit storage limits
	if err := s.FlushAllTransientHandles(); err != nil {
		errs = append(errs, fmt.Errorf("flushing all transient handles: %w", err))
	}
...
func (s *tpmSession) readEKCertFromNVRAM(nvIndex uint32) ([]byte, error) {
	...
	// Read the certificate data using Owner hierarchy
	readCmd := tpm2.NVRead{
		AuthHandle: tpm2.AuthHandle{
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read from NVRAM index 0x%x: %w", nvIndex, err)
	}
