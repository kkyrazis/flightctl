...
func TestSealPassword(t *testing.T) {
	...
	// Initialize persistence for this test
	client.persistence, err = newPersistence(rw, "test_seal.yaml")
	require.NoError(err)
	defer client.Close(context.Background())
	defer simulator.Close()

	pass := []byte("secure password")
	err = client.sealStoragePassword(pass)
	require.NoError(err)
	unsealed, err := client.unsealStoragePassword()
	require.NoError(err)
...
func TestOwnershipEnsureStorageHierarchyPassword(t *testing.T) {
	...
	t.Run("unseals existing password when auth already set", func(t *testing.T) {
		...
		err = client.sealStoragePassword(originalPassword)
		require.NoError(err)
